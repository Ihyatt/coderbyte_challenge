import itertools

def power_set_count(lst):
	"""have the function PowerSetCount(arr) take the array of integers stored in arr, and return the length of the power set 
	(the number of all possible sets) that can be generated. For example: if arr is [1, 2, 3], then the following sets form the power set: 

	[] 
	[1]
	[2]
	[3]
	[1, 2]
	[1, 3]
	[2, 3]
	[1, 2, 3] 

	You can see above all possible sets, along with the empty set, are generated. Therefore, for this input, your program should return 8. 
	Example::
	>>> power_set_count([1, 2, 3, 4])
	16

	>>> power_set_count([5, 6])
	4
	"""
	repeat = []
	non_repeat = []

	idx = 0 
	while idx <= len(lst):
		perms = list(itertools.permutations(lst, idx))
		repeat.append(perms)
		idx += 1
	

	for lists in repeat:
		for sets in lists:
			sets = sorted(list(sets))
			if sets not in non_repeat:
				non_repeat.append(sets)
	return len(non_repeat)



#####################################################################
if __name__ == "__main__":
	print
	import doctest
	if doctest.testmod().failed == 0:
		print "*** ALL TESTS PASSED ***"
   
